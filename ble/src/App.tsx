...
function runCalmness(
	model: WasmCalmnessModel,
	...
	const c = {
		...
		state: r.state_description(),
		alphaBetaRatio: r.alpha_beta_ratio,
	};
	r.free();
	return c;
}

function runAlphaBump(
	model: WasmAlphaBumpDetector,
	samples: Float32Array,
): AlphaBump | null {
	const r = model.process(samples);
	if (!r) return null;
...
function runAlphaPeak(
	model: WasmAlphaPeakModel,
	samples: Float32Array,
): AlphaPeak | null {
	const r = model.process(samples);
	if (!r) return null;
...
function ModeToggle({
	...
	return (
		<div className="mode-toggle">
			<span
				className={`mode-toggle-label${isHeadband ? "" : " mode-toggle-label--active"}`}
			>
				Synthetic
			</span>
			<button
...
export default function App() {
	const [mode, setMode] = useState<Mode>("headband");
	...
	function clearTick() {
		...
		if (synthFastRef.current !== null) {
			clearInterval(synthFastRef.current);
			synthFastRef.current = null;
		}
	}
	...
	function processWindow(samples: Float32Array) {
		...
		if (calmnessRef.current)
			setCalmness(runCalmness(calmnessRef.current, samples));
		if (alphaBumpRef.current)
			setAlphaBump(runAlphaBump(alphaBumpRef.current, samples));
		if (alphaPeakRef.current)
			setAlphaPeak(runAlphaPeak(alphaPeakRef.current, samples));
	}
	...
	useEffect(() => {
		...
		return () => {
			calmnessRef.current?.free();
			calmnessRef.current = null;
			alphaBumpRef.current?.free();
			alphaBumpRef.current = null;
			alphaPeakRef.current?.free();
	...
	useEffect(() => {
		...
		intervalRef.current = setInterval(
			() => processWindow(makeSyntheticWindow()),
			1000,
		);
	...
	function switchMode(next: Mode) {
		...
		resetModels();
		setStatus(
			next === "synthetic" ? "Switching to synthetic…" : "Ready to connect.",
		);
		setMode(next);
	}
	...
	return (
		<div className="app">
			<header className="topbar" aria-label="Application header">
				...
				<span className="topbar-tagline">Muse Web Bluetooth starter</span>
			...
			<div className="mode-bar">
				...
				{mode === "headband" && (
					<div className="connect-group">
						...
						{deviceName && <span className="connect-device">{deviceName}</span>}
						{sampleCount > 0 && (
							<span className="connect-samples">
			...
			<main className="main">
				<section className="stage" aria-labelledby="stage-heading">
					...
					<div className="stage-video-wrap">
						...
						<p className="stage-hint">
							BLE-first starter for Muse-compatible headsets. Use synthetic mode
							only when you want to verify the WASM path without hardware.
						</p>
					</div>
				...
				<p className="deck">
					Built with <code>@elata-biosciences/eeg-web</code> — WASM EEG
					analysis, Muse BLE streaming, native demo references, and a layout
					suited for demos and screen recordings.
				</p>

				<section
					className="native-tools"
					aria-labelledby="native-tools-heading"
				>
					<div className="native-tools-head">
						<p className="native-tools-eyebrow">Native BLE references</p>
						<h2 id="native-tools-heading" className="native-tools-title">
							Companion tools in this repo
						</h2>
						<p className="native-tools-copy">
							Use these repo paths when you need native Bluetooth plumbing
							alongside the browser BLE flow.
						</p>
					</div>

					<div className="native-tools-grid">
						<article className="native-card">
							<p className="native-card-eyebrow">iOS native BLE reference</p>
							<h3 className="native-card-title">CoreBluetooth Muse manager</h3>
							<p className="native-card-copy">
								Full native BLE flow with CoreBluetooth, Muse packet parsing,
								and a SwiftUI demo surface.
							</p>
							<div className="native-card-paths">
								<code>ios-demo/README.md</code>
								<code>
						...
						<article className="native-card">
							<p className="native-card-eyebrow">
							...
							<p className="native-card-copy">
								Native Android starting points in this repo are lighter today,
								but these paths show the demo shell and Bluetooth permission
								wiring.
							</p>
							<div className="native-card-paths">
								<code>android-demo/README.md</code>
								<code>android-demo/app/src/main/AndroidManifest.xml</code>
							</div>
						</article>
					</div>
				</section>
			...
			<footer className="footer">
				<span>Elata SDK · BLE-first EEG web template</span>
			</footer>
		</div>
	);
}
