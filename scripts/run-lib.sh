...
# This file is the shared implementation library for the repo task runner.
# It is sourced by both ./run.sh (thin wrapper) and the justfile recipes.
# ROOT_DIR is the repository root (this library lives in scripts/).
...
verify_script_for_target() {
    case "$1" in
        eeg-web|eeg-web-ble|rppg-web|rppg-models-web|ppg-web|create-elata-demo|app-metrics|app-payments|biosignal-session) echo "verify:publish" ;;
        *)
            echo "Unknown package target: $1" >&2
            return 1
            ;;
...
prepare_script_for_target() {
    case "$1" in
        eeg-web|eeg-web-ble|rppg-web|rppg-models-web|ppg-web|create-elata-demo|app-metrics|biosignal-session) echo "prepare:publish" ;;
        *)
            echo "Unknown package target: $1" >&2
            return 1
            ;;
...
# Command dispatch. Runs at top-level scope when this library is sourced, using
# the caller's positional parameters as the command + args. ./run.sh sources
# this directly; justfile recipes do `set -- <cmd> <args>` first. Keeping this
# at top level (not inside a function) preserves the original variable scoping
# that the `test` command's arrays and EXIT trap rely on.
cmd="${1:-help}"
case "$cmd" in
    install)
        RUN_SH_TASK="install"
