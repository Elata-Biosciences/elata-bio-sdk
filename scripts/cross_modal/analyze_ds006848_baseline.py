#!/usr/bin/env python3
"""Analyze DS006848 expanded-split EEG-to-PPG baseline behavior."""
...
def build_report(*, config_path: Path, metrics: dict) -> str:
    ...
    branch_beats_null = best_branch["aggregate_model_mse_std"] < best_branch["aggregate_null_mse_std"]
    average_amplitude_delta = float(
        np.mean(
            [
                row["family_delta_mse_std"]["amplitude_family"]
                for row in best_branch["subject_aggregate"].values()
            ]
        )
    )
    average_timing_delta = float(
        np.mean(
            [
                row["family_delta_mse_std"]["timing_family"]
                for row in best_branch["subject_aggregate"].values()
            ]
        )
    )
    dominant_family = "amplitude" if average_amplitude_delta >= average_timing_delta else "timing"
    headline_outcome = "beats" if branch_beats_null else "does not beat"
    lines = [
        ...
        f"- The current best branch is `{recommended_branch}` and it {headline_outcome} the null on aggregate standardized MSE.",
        f"- The dominant error concentration is currently in the {dominant_family}-style target family.",
        "",
    ]
    lines.extend(
        [
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Analyze DS006848 expanded-split EEG-to-PPG baseline behavior.")
    parser.add_argument(
        "--config",
    ...
    for branch_name in feature_branches:
        ...
        for target_name in target_names:
            ...
            eval_local_lookup = {int(row_index): local_index for local_index, row_index in enumerate(eval_row_indices.tolist())}
            ...
            for subject_id in eval_subject_ids:
                subject_mask = np.asarray([row["subject_id"] == subject_id for row in metadata], dtype=bool) & eval_mask
                subject_rows = np.flatnonzero(subject_mask)
                local_indices = [eval_local_lookup[int(row_index)] for row_index in subject_rows.tolist()]
    ...
    print(f"Wrote DS006848 baseline analysis metrics to {metrics_path}")
    print(f"Wrote DS006848 baseline analysis report to {report_path}")
    return 0
