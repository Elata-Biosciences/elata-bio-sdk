#!/usr/bin/env python3
"""Validate DS006848 calibrated hybrid detail baseline outputs."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate DS006848 calibrated hybrid detail baseline outputs.")
    parser.add_argument(
        "--config",
    ...
    for key in ["raw_feature_dim", "detail_feature_dim", "combined_feature_dim"]:
        if not isinstance(metrics.get(key), int) or int(metrics[key]) <= 0:
            failures.append(f"{key} in metrics must be a positive integer.")
    if metrics.get("combined_feature_dim", 0) != metrics.get("raw_feature_dim", 0) + metrics.get("detail_feature_dim", 0):
        failures.append("Combined feature dimension must equal raw plus detail dimensions.")
    ...
    print("Validated DS006848 calibrated hybrid detail baseline outputs.")
    return 0
