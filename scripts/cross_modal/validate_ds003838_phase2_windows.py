#!/usr/bin/env python3
"""Validate DS003838 pilot Phase 2 paired window outputs."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate DS003838 pilot Phase 2 paired window outputs.")
    parser.add_argument(
        "--config",
    ...
    if np.isnan(arrays["eeg_event_windows"]).any() or np.isnan(arrays["eeg_clean_windows"]).any():
        failures.append("Found NaNs in EEG tensors.")
    if np.isnan(arrays["ppg_native_windows"]).any() or np.isnan(arrays["ppg_clean_windows"]).any():
        failures.append("Found NaNs in PPG tensors.")
    ...
    print("Validated DS003838 Phase 2 paired windows.")
    return 0
