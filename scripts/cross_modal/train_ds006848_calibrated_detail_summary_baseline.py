#!/usr/bin/env python3
"""Train DS006848 calibrated multiscale detail-summary baselines on amplitude targets."""
...
def haar_decompose_last_axis(array: np.ndarray, levels: int) -> tuple[np.ndarray, list[np.ndarray]]:
    approximation = array.astype(np.float64, copy=False)
    details: list[np.ndarray] = []
    for _ in range(levels):
        length = approximation.shape[-1]
        if length % 2 != 0:
            raise SystemExit(f"Haar decomposition requires even axis lengths at every level; found length {length}.")
...
def summarize_detail_band(detail: np.ndarray, statistics: list[str]) -> list[np.ndarray]:
    summaries: list[np.ndarray] = []
    abs_detail = np.abs(detail)
    for statistic in statistics:
        if statistic == "rms":
            summaries.append(np.sqrt(np.mean(detail**2, axis=-1)))
        elif statistic == "max_abs":
            summaries.append(np.max(abs_detail, axis=-1))
        elif statistic == "mean_abs":
            summaries.append(np.mean(abs_detail, axis=-1))
        else:
            raise SystemExit(f"Unsupported detail statistic: {statistic}")
    return summaries


def detail_summary_feature_view(features: np.ndarray, levels: int, statistics: list[str], include_final_approx: bool) -> np.ndarray:
    window_length = features.shape[-1]
    if window_length <= 0 or (window_length & (window_length - 1)) != 0:
        raise SystemExit(
            f"Detail-summary feature view requires power-of-two window length; found {window_length} for source branch."
        )
    max_levels = int(np.log2(window_length))
    if levels < 1 or levels > max_levels:
        raise SystemExit(f"Requested detail-summary levels {levels} but valid range is 1..{max_levels}.")

    approximation, details = haar_decompose_last_axis(features, levels)
    summary_blocks: list[np.ndarray] = []
    for detail in details:
        summary_blocks.extend(summarize_detail_band(detail, statistics))
    if include_final_approx:
        summary_blocks.append(approximation.reshape(approximation.shape[0], approximation.shape[1]))
    return np.concatenate(summary_blocks, axis=1)
...
def build_report(*, config_path: Path, metrics: dict) -> str:
    lines = [
        ...
        f"- Detail statistics: `{', '.join(metrics['detail_statistics'])}`",
        f"- Include final approximation: `{metrics['include_final_approx']}`",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train DS006848 calibrated detail-summary baselines.")
    parser.add_argument(
        "--config",
    ...
    detail_statistics = [str(name) for name in config["feature_view"]["detail_statistics"]]
    include_final_approx = bool(config["feature_view"]["include_final_approx"])
    ...
    transformed = detail_summary_feature_view(
        source_features,
        haar_levels,
        detail_statistics,
        include_final_approx,
    )
    ...
    if not valid_candidate_ranks:
        raise SystemExit("No valid candidate ranks for transformed detail-summary feature view.")
    ...
    metrics = {
        ...
        "detail_statistics": detail_statistics,
        "include_final_approx": include_final_approx,
    ...
    print(f"Wrote calibrated detail-summary baseline report to {report_path}")
    print(f"Wrote calibrated detail-summary baseline metrics to {metrics_path}")
    return 0
