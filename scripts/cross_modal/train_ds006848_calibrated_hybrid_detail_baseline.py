#!/usr/bin/env python3
"""Train DS006848 calibrated hybrid raw-plus-detail baselines on amplitude targets."""
...
from train_ds006848_calibrated_detail_summary_baseline import (
    detail_summary_feature_view,
    low_rank_reference_summary,
)
...
def build_report(*, config_path: Path, metrics: dict) -> str:
    lines = [
        ...
        f"- Raw feature dim: `{metrics['raw_feature_dim']}`",
        f"- Detail feature dim: `{metrics['detail_feature_dim']}`",
        f"- Combined feature dim: `{metrics['combined_feature_dim']}`",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train DS006848 calibrated hybrid detail baselines.")
    parser.add_argument(
        "--config",
        default="configs/cross_modal/ds006848_calibrated_hybrid_detail_amplitude_cohort_swap.toml",
    ...
    raw_features = source_features.reshape(source_features.shape[0], -1)
    detail_features = detail_summary_feature_view(
        source_features,
        haar_levels,
        detail_statistics,
        include_final_approx,
    )
    combined_features = np.concatenate([raw_features, detail_features], axis=1)
    ...
    x_means, x_scales, vt = compute_basis(combined_features, train_quality_mask)
    standardized = standardize(combined_features, x_means, x_scales)
    ...
    if not valid_candidate_ranks:
        raise SystemExit("No valid candidate ranks for transformed hybrid feature view.")
    ...
    metrics = {
        ...
        "detail_feature_dim": int(detail_features.shape[1]),
    ...
    for rank in valid_candidate_ranks:
        ...
        for target_name in target_names:
            ...
            rank_metrics["per_target"][target_name] = {
                ...
                "model_mse": model_mse,
                "null_mse": null_mse,
                "relative_mse": relative_mse_value,
                "model_mae": model_mae,
                "null_mae": null_mae,
                "relative_mae": relative_mae_value,
    ...
    print(f"Wrote calibrated hybrid detail baseline report to {report_path}")
    print(f"Wrote calibrated hybrid detail baseline metrics to {metrics_path}")
    return 0
