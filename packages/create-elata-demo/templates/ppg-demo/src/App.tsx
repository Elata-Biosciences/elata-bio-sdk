...
export default function App() {
	...
	const [diagnostics, setDiagnostics] = useState<PpgSessionDiagnostics | null>(
		null,
	);
	...
	async function handleConnect() {
		...
		try {
			...
			setStatus(
				error instanceof Error ? error.message : "Failed to connect to Muse.",
			);
		} finally {
			setBusy(false);
		}
	...
	return (
		<div className="app">
			<section className="hero">
				...
				<p>
					This template connects over Web Bluetooth, starts{" "}
					<code>createMusePpgSession()</code>, selects the best available PPG
					channel automatically, and renders BPM, RMSSD, SDNN, and the live
					waveform for fast developer iteration.
				</p>
					...
					<section className="panel pad metrics" aria-label="Metrics">
						<article className="metric-card">
							...
							<div className="metric-value">
								{formatNumber(metrics?.bpm, 0)}
							</div>
							<div className="metric-sub">
								Confidence{" "}
								{formatNumber(
									(metrics?.confidence ?? null) != null
										? (metrics?.confidence ?? 0) * 100
										: null,
									0,
									"%",
								)}
						...
						<article className="metric-card">
							...
							<div className="metric-value">
								{formatNumber(metrics?.rmssdMs, 0, " ms")}
							</div>
							<div className="metric-sub">
						...
						<article className="metric-card">
							...
							<div className="metric-value">
								{formatNumber(
									(metrics?.signalQuality ?? null) != null
										? (metrics?.signalQuality ?? 0) * 100
										: null,
									0,
									"%",
								)}
					...
					<section className="panel wave-panel">
						<div className="wave-header">
							<div>
								...
								<p>
									Selected channel waveform from the current processing window.
								</p>
							</div>
							<p>
								Window {formatNumber(metrics?.windowDurationMs, 0, " ms")} /
								Sample rate {formatNumber(metrics?.sampleRateHz, 0, " Hz")}
							</p>
						</div>
						<canvas
							ref={waveformRef}
				...
				<div className="stack">
					<section className="panel pad meta-grid" aria-label="Diagnostics">
						...
						<article className="meta-card">
							...
							<span>
								{metrics?.reasonCodes?.length
									? metrics.reasonCodes.join(", ")
									: "none"}
							</span>
						</article>
						<article className="meta-card">
						...
						<article className="meta-card">
							...
							<span>
								{formatNumber(deferredDiagnostics?.lastSampleAgeMs, 0, " ms")}
							</span>
						</article>
					</section>
					...
					<section className="panel pad tips">
						<h2>Capture tips</h2>
						<p>
							The starter keeps the BLE and DSP wiring high-level on purpose. It
							is built to be the shortest path from a Muse device to inspectable
							HRV metrics.
						</p>
						<ul>
							<li>
								<strong>Keep the band stable.</strong>
								Motion still affects channel selection and HRV confidence more
								than BPM.
							</li>
							<li>
								<strong>Prefer Athena optics when available.</strong>
								Those packets already carry device timestamps through the
								decoder path.
							</li>
							<li>
								<strong>Watch reason codes.</strong>
								They surface when the window is still warming up, signal quality
								is low, or the pulse estimators disagree.
							</li>
						</ul>
					</section>
				</div>
