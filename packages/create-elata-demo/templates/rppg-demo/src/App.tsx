...
function getStatusTone(
	...
	if (
		!diagnostics ||
		diagnostics.backendMode !== "wasm" ||
		!diagnostics.estimationAvailable
	) {
		return "warn";
	}
...
export default function App() {
	...
	const [diagnostics, setDiagnostics] = useState<RppgSessionDiagnostics | null>(
		null,
	);
	...
	const syncFromSession = useCallback(() => {
		...
		tracker.observePhysiology(
			nextMetrics.bpm ?? null,
			nextMetrics.hrv_rmssd ?? null,
		);
			...
			try {
				...
				const message =
					error instanceof Error
						? error.message
						: "Could not start the rPPG session.";
				setStatus(message);
			}
		}
	...
	const readinessLabel =
		diagnostics?.estimationAvailable && metrics.bpm != null
			? "Ready"
			: "Warm-up";
	...
	return (
		<div className="app">
			...
			<main className="main">
				<section className="stage" aria-labelledby="stage-heading">
					...
					<div className="stage-video-wrap">
						...
						<p className="stage-hint">
							Face the light, fill the frame, and stay steady for the first few
							seconds.
						</p>
					</div>
						...
						<ul className="chip-row" aria-label="Session state">
							<li className="chip">
								...
								<span className="chip-val">
									{diagnostics?.backendMode ?? "…"}
								</span>
							</li>
							<li className="chip">
								<span className="chip-key">Tracking</span>
								<span className="chip-val">
									{diagnostics?.faceTrackingMode ?? "…"}
								</span>
							</li>
							<li className="chip">
								<span className="chip-key">ROI</span>
								<span className="chip-val">
									{diagnostics?.roiSource ?? "—"}
								</span>
							</li>
						</ul>
					</aside>
				...
				<p className="deck">
					Built with <code>createRppgSession()</code> — packaged WASM, live
					diagnostics, and a layout suited for demos and screen recordings.
				</p>
					...
					<div className="panel-inner">
						<div className="stats-grid">
							...
							<div className="stat-row">
								...
								<span className="stat-value">
									{formatInteger(diagnostics?.framesSeen ?? 0)}
								</span>
							</div>
							<div className="stat-row">
							...
							<div className="stat-row stat-row--wide">
								...
								<span className="stat-value">
									{formatIssueList(diagnostics?.issues)}
								</span>
							</div>
							<div className="stat-row stat-row--wide">
						...
						<ol className="guidance-list">
							<li className="guidance-item">
								...
								<div>
									...
									<span>
										Even lighting beats resolution for a stable trace.
									</span>
								</div>
							</li>
							<li className="guidance-item">
								...
								<div>
									...
									<span>
										Give the estimator a few calm seconds before reading BPM.
									</span>
								</div>
							</li>
							<li className="guidance-item">
								...
								<div>
									...
									<span>
										Backend should read <code>wasm</code>. If not, verify Vite
										URLs for the packaged <code>.wasm</code> assets.
									</span>
								</div>
							</li>
						</ol>
