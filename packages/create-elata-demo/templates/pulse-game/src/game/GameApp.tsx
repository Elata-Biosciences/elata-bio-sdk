...
export default function GameApp() {
	...
	const [savedBaseline, setSavedBaseline] = useState<BaselineRecord | null>(
		null,
	);
	...
	const triggerStartle = useCallback((baselineBpm: number) => {
	...
	return (
		<div className="game">
			{flashActive && (
				<div
					className={
						reducedMotion ? "flash flash-reduced" : "flash flash-bright"
					}
					aria-hidden
				/>
			)}
			...
			return (
				<div className="phase">
					...
					<ol>
						...
						<li>
							Brief visual burst on screen (toggle reduced-motion above if
							photosensitive).
						</li>
					...
					{savedBaseline && (
						<p className="hint">
							Saved baseline: {Math.round(savedBaseline.restingBpm)} BPM.
							Calibrating again replaces it.
						</p>
					)}
		...
		case "ready":
			return (
				<div className="phase">
					...
					<p>
						When you press start, a random 5–15s wait will pass, then a brief
						visual burst. Stay seated; we’ll measure how fast your pulse
						returns.
					</p>
