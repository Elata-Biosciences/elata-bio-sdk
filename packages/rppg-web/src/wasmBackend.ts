...
export async function loadWasmBackend(importer: WasmImporter = defaultImporter): Promise<Backend | null> {
  const candidates = ['/pkg/rppg_wasm.js', '/pkg/eeg_wasm.js', '/rppg_wasm.js', '/eeg_wasm.js'];
  for (const url of candidates) {
    try {
      ...
      if (mod.default && typeof mod.default === 'function') {
        return { newPipeline: (sr: number, ws: number) => new (mod.default as any)(sr, ws) };
      }
    } catch (e) {
      // ignore and try next
    }
