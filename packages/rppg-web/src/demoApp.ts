...
export async function initDemo(videoEl: HTMLVideoElement, opts: DemoRunnerOptions & { sampleRate?: number; windowSec?: number } = {}) {
  ...
  if (!backend) {
    ...
    backend = { newPipeline: (sr: number, w: number) => ({ push_sample: () => {}, get_metrics: () => ({ bpm: null, confidence: 0, signal_quality: 0 }) }) };
  }
  ...
  (window as any).__rppg_demo = { source, proc, runner, backendAvailable: !!(backend && backend.newPipeline) };
