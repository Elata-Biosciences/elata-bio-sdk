...
export class MuseBleDevice {
	...
	private ppgBuffers: Record<string, number[]> = {
		PPG1: [],
		PPG2: [],
		PPG3: [],
	};
	...
	private onPpgCallback:
		| ((
				...
				packet:
					| PpgPacket
					| AthenaAuxPacket
					| { sequence: number; ir: number[]; nearIr: number[]; red: number[] },
		  ) => void)
		| null = null;
	...
	private splitPpgSamples(samples: number[]): {
		ir: number[];
		nearIr: number[];
		red: number[];
	} {
	...
	async prepareSession(): Promise<void> {
		...
		this.availableCharacteristics = characteristics.map(
			(char: BluetoothRemoteGATTCharacteristic) => char.uuid.toLowerCase(),
		);
		const hasAthena =
