...
function dominantInBand(values, sampleRate, minHz = RESP_MIN_HZ, maxHz = RESP_MAX_HZ) {
...
function resampleTachogram(ibisMs, beatTimesMs, resampleHz = RSA_RESAMPLE_HZ) {
  if (ibisMs.length < RSA_MIN_BEATS - 1 || beatTimesMs.length < RSA_MIN_BEATS || resampleHz <= 0) {
    return null;
  }
  const pts = [];
...
function analyzePulseWindow(samples) {
  ...
  const respirationConfidence = respirationResult ? respirationResult.confidence : null;
  const skinMean = samples.reduce((acc, sample) => acc + clamp3(sample.skinRatio, 0, 1), 0) / n;
  const motionMean = samples.reduce((acc, sample) => acc + clamp3(sample.motion, 0, 1), 0) / n;
  const clipMean = samples.reduce((acc, sample) => acc + clamp3(sample.clipRatio, 0, 1), 0) / n;
...
function estimateDominantBpm(data, sampleRate, minHz, maxHz) {
  ...
  const confidence = energy > 1e-9 ? clamp3(bestMag / (Math.sqrt(energy) + 1e-9), 0, 1) : 0;
    ...
    for (let channelIndex = 0; channelIndex < block.channelCount; channelIndex++) {
      ...
      for (let rowIndex = 0; rowIndex < rowCount; rowIndex++) {
        ...
        stream.samples.push({
          ...
          intensity,
          skinRatio: 1,
          motion: prev && stream.motionScale > 0 ? clamp4(delta / (stream.motionScale * 4), 0, 1) : 0,
          clipRatio: 0
        });
...
function drawWaveform() {
  ...
  if (visiblePoints.length > 1) {
    ...
    visiblePoints.forEach((point, index) => {
      ...
      const normalizedValue = clamp5(displayValues[index] ?? 0, visualMin, visualMax);
