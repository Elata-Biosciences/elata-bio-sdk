...
- the EEG WASM runtime initialized before connecting (`initEegWasm()` from `eeg-web`) — `AthenaWasmDecoder` is a WASM class and will fail if the runtime is not ready
...
export async function connectHeadband() {
  // Initialize the EEG WASM runtime before connecting — required for AthenaWasmDecoder.
  await initEegWasm();
...
**`stop()` vs `disconnect()`:** `stop()` ends the data stream but keeps the
BLE connection alive — useful if you plan to call `startStreaming()` again
shortly. `disconnect()` fully closes the BLE connection and removes the
device pairing for this session. For a route or component exit, `stop()` is
usually enough; call `disconnect()` if you want to fully release the device.
