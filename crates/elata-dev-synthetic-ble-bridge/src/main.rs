...
fn run_ble_muse_classic(
    ...
    use elata_muse_proto::classic::{characteristic, encode_eeg_packet, spec};
    use elata_muse_proto::SERVICE_UUID_U128;
                    ...
                    if let Ok(async_op) = args.GetRequestAsync() {
                        if let Ok(request) = pollster::block_on(async_op) {
                            if let Ok(value) = request.Value() {
                                ...
                                // Parse command
                                if let Some(cmd) = elata_muse_proto::command::decode(&data) {
                                    match cmd {
                                        "d" => {
    ...
    eprintln!("Service UUID: {}", elata_muse_proto::SERVICE_UUID);
...
fn run_ble_muse_athena(
    ...
    use elata_muse_proto::athena::{characteristic, command, encode_eeg_packet, spec, EegChannels};
    use elata_muse_proto::SERVICE_UUID_U128;
    use std::sync::atomic::{AtomicBool, Ordering};
...
fn run_ble_legacy(
    ...
    use proto::{EEG_STREAM_CHAR_UUID_U128, INFO_CHAR_UUID_U128, SERVICE_UUID_U128};
...
mod tests {
    ...
    fn ble_athena_defaults_override_manual_settings() {
        ...
        assert_eq!(
            cfg.sample_rate_hz,
            elata_muse_proto::athena::spec::EEG_SAMPLE_RATE
        );
        assert_eq!(
            cfg.channel_count,
            elata_muse_proto::athena::spec::EEG_CHANNEL_COUNT
        );
        assert_eq!(
            cfg.samples_per_channel,
