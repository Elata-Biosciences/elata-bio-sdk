...
mod tests {
    ...
    fn sample_packet() -> EegPacket {
        EegPacket {
            ...
            data: vec![0.25, -1.5, 2.0, 3.25, -4.5, 5.75],
        }
    }

    #[test]
    fn packet_len_matches_header_plus_payload() {
        let packet = sample_packet();
        assert_eq!(
            packet_len(packet.channel_count, packet.samples_per_channel),
            HEADER_LEN + packet.data.len() * 4
        );
        assert_eq!(packet.expected_sample_count(), packet.data.len());
    }

    #[test]
    fn encode_decode_round_trip_preserves_packet() {
        let packet = sample_packet();
        let mut encoded = vec![9, 9, 9];
    ...
    #[test]
    fn decode_rejects_too_short_packets() {
        let err = decode_packet(&[0_u8; HEADER_LEN - 1]).expect_err("packet should be too short");
        assert_eq!(err, DecodeError::TooShort);
    }

    #[test]
    fn decode_rejects_invalid_payload_length() {
        let packet = sample_packet();
        let mut encoded = Vec::new();
        encode_packet(&packet, &mut encoded);
        encoded.pop();

        let err = decode_packet(&encoded).expect_err("packet length should be invalid");
        assert_eq!(
            err,
            DecodeError::InvalidLength {
                expected: packet_len(packet.channel_count, packet.samples_per_channel),
                actual: encoded.len(),
            }
        );
    }
    ...
    #[test]
    fn encode_info_json_escapes_special_characters() {
        let info = InfoPayload {
            device_name: "Muse \"Synthetic\"\nBridge\\Test",
            channel_count: 4,
            sample_rate_hz: 256,
        ...
        assert_eq!(
            json,
            "{\"device_name\":\"Muse \\\"Synthetic\\\"\\nBridge\\\\Test\",\"channel_count\":4,\"sample_rate_hz\":256,\"samples_per_channel\":12}"
        );
    }
}
