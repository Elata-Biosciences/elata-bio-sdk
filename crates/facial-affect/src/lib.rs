...
impl SentimentCore {
    pub fn new(cfg: SentimentConfig) -> Self {
        ...
        Self {
            cfg,
            buffer: VecDeque::with_capacity(smooth_cap),
            baseline_buffer: VecDeque::with_capacity(baseline_cap),
            baseline: None,
            last_metrics: None,
        }
    }
    ...
    pub fn finalize_calibration(&mut self) {
        if self.baseline_buffer.is_empty() {
            self.baseline = None;
            return;
        }
        ...
        for m in &self.baseline_buffer {
            sum.smile += m.smile;
            sum.brow_height += m.brow_height;
            sum.brow_distance += m.brow_distance;
            sum.eye_openness += m.eye_openness;
        }
        let n = self.baseline_buffer.len() as f32;
        self.baseline = Some(LandmarkMetrics {
            smile: sum.smile / n,
            brow_height: sum.brow_height / n,
            brow_distance: sum.brow_distance / n,
            eye_openness: sum.eye_openness / n,
        });
    }
    ...
    pub fn update_from_metrics(
        ...
        if self.buffer.len() > self.cfg.smooth_window {
        ...
        if self.baseline_buffer.capacity() == self.cfg.baseline_window {
            ...
            if self.baseline.is_none() {
                ...
                if self.baseline_buffer.len() > self.cfg.baseline_window {
        ...
        for m in &self.buffer {
            s.smile += m.smile;
            s.brow_height += m.brow_height;
            s.brow_distance += m.brow_distance;
            s.eye_openness += m.eye_openness;
        }
        ...
        let delta_smile = match self.baseline {
            Some(b) => s.smile - b.smile,
            None => s.smile,
        };
        let delta_brow_height = match self.baseline {
            Some(b) => s.brow_height - b.brow_height,
            None => s.brow_height,
        };
        let delta_brow_distance = match self.baseline {
            Some(b) => s.brow_distance - b.brow_distance,
            None => s.brow_distance,
        };
        let delta_eye = match self.baseline {
            Some(b) => s.eye_openness - b.eye_openness,
            None => s.eye_openness,
        };
        ...
        if delta_smile > self.cfg.smile_threshold {
            label = SentimentLabel::Happy;
            conf = conf.max((delta_smile / (self.cfg.smile_threshold * 2.0)).min(1.0));
        }
        // surprised: brow height spike (sustained)
        if delta_brow_height > self.cfg.brow_height_threshold {
            label = SentimentLabel::Surprised;
            conf = conf.max((delta_brow_height / (self.cfg.brow_height_threshold * 2.0)).min(1.0));
        }
        // stressed: brow distance drop or lowered brow height (sustained)
        if delta_brow_distance < -self.cfg.brow_distance_threshold
            || delta_brow_height < -self.cfg.brow_height_threshold
        {
            label = SentimentLabel::Stressed;
            conf = conf.max(
                ((-delta_brow_distance).max(-delta_brow_height)
                    / (self.cfg.brow_distance_threshold * 2.0))
                    .min(1.0),
            );
        }
        // focused: eye openness high
        if delta_eye > self.cfg.eye_openness_threshold {
            label = SentimentLabel::Focused;
            conf = conf.max((delta_eye / (self.cfg.eye_openness_threshold * 2.0)).min(1.0));
        }
        // fatigue: sustained low eye openness
        if delta_eye < -self.cfg.eye_openness_threshold {
            label = SentimentLabel::Fatigue;
            conf = conf.max(((-delta_eye) / (self.cfg.eye_openness_threshold * 2.0)).min(1.0));
        }
...
mod tests {
    ...
    fn test_smile_detection() {
        ...
        let mut res = core.update_from_metrics(
            LandmarkMetrics {
                smile: 0.0,
                brow_height: 0.0,
                brow_distance: 0.0,
    ...
    fn test_stress_by_brow_drop() {
        ...
        let res = core.update_from_metrics(
            LandmarkMetrics {
                smile: 0.0,
                brow_height: 0.3,
                brow_distance: 0.3,
