...
mod tests {
    ...
    #[test]
    fn test_filter_reset_restores_initial_response() {
        let mut filter = BiquadFilter::lowpass(10.0, 256.0, 0.707);
        let input = [1.0, 0.0, 0.0, 0.0];

        let first_pass = filter.filter(&input);
        filter.reset();
        let second_pass = filter.filter(&input);
