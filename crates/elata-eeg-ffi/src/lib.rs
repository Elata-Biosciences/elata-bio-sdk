...
impl From<elata_eeg_models::AlphaState> for AlphaState {
    fn from(state: elata_eeg_models::AlphaState) -> Self {
        match state {
            elata_eeg_models::AlphaState::High => AlphaState::High,
            elata_eeg_models::AlphaState::Low => AlphaState::Low,
            elata_eeg_models::AlphaState::Transitioning => AlphaState::Transitioning,
            elata_eeg_models::AlphaState::Unknown => AlphaState::Unknown,
        }
    }
}
...
pub struct RppgPipelineFFI {
    inner: Mutex<elata_rppg::RppgPipeline>,
}
