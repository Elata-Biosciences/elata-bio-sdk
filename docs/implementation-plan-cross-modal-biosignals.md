...
- Phase 3 has only a preview implementation so far:
  ...
  - `DS006848` now also has both a first positive-check baseline and a broader follow-up:
    ...
    - the first channel-preserving detail-summary follow-on now also exists:
      - it kept the same accepted amplitude cohorts, the same calibrated low-rank family, and the same short-calibration contract
      - it replaced raw `eeg_clean_windows` with per-channel multiscale detail summaries:
        - detail `rms`
        - detail `max_abs`
        - final approximation scalar
      - it is materially better than the full Haar rotation and slightly beats the null aggregate on both accepted amplitude cohorts:
        - cohort-swap aggregate relative MSE is about `0.9902`
    ...
    - the first hybrid full-morphology follow-on now also exists:
      - the hybrid raw-plus-detail feature view was tested on the broader calibrated morphology benchmark for both `cohort_swap` and `cohort_plus_sub011`
      - on `cohort_swap`, the best hybrid rank is `16` and aggregate relative MSE improves from the low-rank morphology predecessor at about `0.9199` to about `0.8095`
    ...
    - the first cautious shared-rank full-morphology `sub-025` expansion now also exists:
      - it adds `sub-025` to eval under the shared hybrid raw-plus-detail default
      - the split-matched low-rank morphology reference on `eeg_clean` only edges below null at about `0.9975`
      - the hybrid best rank stays `16` and full aggregate relative MSE improves to about `0.8525`
