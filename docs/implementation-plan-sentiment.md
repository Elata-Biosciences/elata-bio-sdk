...
Status: In progress — **partial implementation exists** in **`crates/facial-affect`** (Rust core + calibration). See also **[architecture-sentiment.md](architecture-sentiment.md)**. Phases below still describe a TS-first MediaPipe path; align any new work with the crate and architecture doc before duplicating APIs.
